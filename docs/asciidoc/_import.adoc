...
he `liteOntoImport` procedure takes the same generic params described in <<common_params>> at the begining of the <<Import>> section, so we will invoke it with a URL and a serialisation format. In the following example we'll import the ontology in link:docs/rdf/volkswagen.owl[this file].

[source,cypher]
----
CALL semantics.liteOntoImport("...","Turtle")
---- 
...
[source,cypher]
----
CALL semantics.streamRDF("https://raw.githubusercontent.com/motools/musicontology/master/examples/clapton_perf/clapton.n3","Turtle")
---- 
...
The procedure is read-only and nothing is written to the graph, however, it is possible to use the output to write to the Graph with cypher like in the following example:
