...
**Note** that for this method to run, a uniqueness constraint needs to exist on property `uri` of
nodes labeled as `Resource` so if not done it yet, just run the following command on your
DB or the RDF import procedures will remind you with an error messsage.
...
There is another pre-requisite before carrying out any data import into Neo4j with neosemantics: we need to
define the type and characteristics of the graph we will be creating. These are kept in the `GrapConfig`.

[WARNING]
If we try to run any RDF import procedure without having defined the uniqueness constraint or the GraphConfig first,
the import will fail showing an error message indicating what's missing.

The method `semantics.setGraphConfig` can help us with this setup. Calling the procedure
without parameters sets all the default values:

[source,cypher]
----
call semantics.graphconfig.set()
----

Setting specific values for configuration items is done by passing a map as parameter to same procedure but we'll
see that in detail later in section <<graphconfig>>.
