...
Just like we did in the previous section, we define a vocabulary with `mapping.addSchema` and then we add individual mappings for elements in the vocabulary with `mapping.addMappingToSchema`. If there were multiple vocabularies to map, we would just need to repeat the process for each of them.
