...
== Value range constraints and type mismatches

The SHACL value range constraints (`sh:minInclusive`, `sh:maxInclusive`, `sh:minExclusive`, `sh:maxExclusive`) validate that property values fall within a numeric range. If a property holds a non-numeric string value, the comparison is non-comparable and the constraint is violated, regardless of whether a `sh:datatype` constraint is also present.
...
A node `(:Score { value: "not-a-number" })` will produce a `ValueRangeConstraintComponent` violation because the string value cannot be compared to the numeric bounds. A node `(:Score { value: 101 })` will similarly produce a violation because 101 > 100.

When both `sh:datatype` and range constraints are defined on the same property, both constraints are evaluated independently per the SHACL specification. A value that violates both (e.g. a string `"200"` where an integer in 0–100 is expected) will produce two violations: one for `DatatypeConstraintComponent` and one for `ValueRangeConstraintComponent`.
