...
public class MappingUtils {
    ...
    public Stream<StringOutput> dropSchema(@Name("schemaUri") String schemaUri) {
        ...
        if (!schemas.hasNext()){
            ...
            Iterable<Relationship> inRels = schemaToDelete.getRelationships(RelationshipType.withName("_IN"), Direction.INCOMING);
            inRels.forEach(x -> { x.getOtherNode(schemaToDelete).delete(); x.delete(); });
