...
abstract class RDFDatasetToLPGStatementProcessor implements RDFHandler {
  ...
  RDFDatasetToLPGStatementProcessor(GraphDatabaseService db, RDFParserConfig conf) {
    this.graphdb = db;
   ...
   */
  private Object getObjectValue(IRI propertyIRI, Literal object) {
  ...
  private boolean setProp(ContextResource contexedResource, IRI propertyIRI,
    ...
    if (propValue != null) {
      if (!resourceProps.containsKey(contexedResource)) {
        props = initialiseProps(contexedResource);
        initialiseLabels(contexedResource);
      } else {
        props = resourceProps.get(contexedResource);
      }
  ...
  private void setLabel(ContextResource contexedResource, String label) {
    ...
    if (!resourceLabels.containsKey(contexedResource)) {
      initialiseProps(contexedResource);
      labels = initialiseLabels(contexedResource);
    } else {
      labels = resourceLabels.get(contexedResource);
    }
