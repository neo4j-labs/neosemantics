  ...
  public void testGetNodeById() throws Exception {
    ...
    try(Transaction tx =  graphDatabaseService.beginTx()) {
      ...
      HTTP.Response response = HTTP.withHeaders("Accept", "application/ld+json").GET(
          HTTP.GET(neo4j.httpURI().resolve("rdf").toString()).location() + "describe/id/"
              + id.toString());
  ...
  public void ImportGetNodeById() throws Exception {
    ...
    try (Transaction tx = graphDatabaseService.beginTx()) {
      ...
      try (Driver driver = GraphDatabase.driver(neo4j.boltURI(),
        ...
        org.neo4j.driver.Result postImport = session.run("MATCH (n:Critic) RETURN n");
        Node criticPostImport = postImport.next().get("n").asNode();

        try (Transaction tx = graphDatabaseService.beginTx()) {

          Result result = tx.execute("MATCH (n:Critic) "
